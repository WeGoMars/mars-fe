...
export default function StockDetails({ symbol, activeTab, onTabChange, favoriteStocks, setFavoriteStocks, isLoggedIn }: StockDetailsProps) {
  ...
  const { data: portfolioData, isLoading: isPortfolioLoading, refetch: refetchPortfolio } = useGetOverallPortfolioQuery();
          ...
          <div className="flex justify-between gap-1 md:gap-2 mb-4 overflow-x-auto">
            {(['종목정보 상세', '내 계좌', 'AI 추천'] as const).map((tab) => (
              <button
                ...
                className={`px-2 md:px-4 py-2 rounded-xl font-medium text-xs md:text-sm whitespace-nowrap transition-colors ${activeTab === tab ? 'bg-[#f5f7f9]' : 'bg-[#f5f7f9] hover:bg-gray-200'
                  }`}
              >
                {tab}
              </button>
            ...
            <>
              ...
              <div className="bg-[#e7f4e8] rounded-2xl p-4 mb-6">
                ...
                <div className="flex gap-2">
                  <button
                    className={`flex-1 rounded-xl py-2 px-2 flex items-center justify-center gap-1 whitespace-nowrap ${selectedRiskLevel === 'high' ? 'bg-[#ffe2e5]' : 'bg-[#ffe2e5] opacity-50'
                      }`}
                  ...
                  <button
                    className={`flex-1 rounded-xl py-2 px-2 flex items-center justify-center gap-1 whitespace-nowrap ${selectedRiskLevel === 'low' ? 'bg-[#c3e7f2]' : 'bg-[#c3e7f2] opacity-50'
                      }`}
              ...
              <div className="bg-[#fff4e4] rounded-2xl p-4 mb-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {[
                    ...
                    <button
                      key={strategy.value}
                      className={`flex-1 bg-[#ffffff] rounded-xl py-2 px-2 whitespace-nowrap ${selectedStrategies.includes(strategy.value as PreferredStrategy) ? 'border-2 border-[#006ffd]' : ''
                        }`}
              ...
              <div className="bg-[#c3e7f2] rounded-2xl p-4 mb-6">
                ...
                <div className="grid grid-cols-2 gap-2 max-h-32 overflow-y-auto overflow-x-hidden">
                  {[
                    ...
                    <button
                      key={sector.value}
                      className={`bg-[#ffffff] rounded-xl py-1 px-2 whitespace-nowrap ${selectedSectors.includes(sector.value as PreferredSector) ? 'border-2 border-[#006ffd]' : ''
                        }`}
              ...
              <div className="bg-[#f4f5f9] rounded-2xl p-4 text-center">
                ...
                <button
                  className={`text-[#71727a] font-medium text-sm tracking-tight border-2 border-[#2563eb] rounded-md px-4 py-2 ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''
                    }`}
                    ...
                    <div className="bg-[#e7f4e8] rounded-2xl p-4">
                      ...
                      <div className="space-y-4">
                        <div>
                          ...
                          <div className="flex gap-2">
                            <button className={`flex-1 rounded-xl py-2 px-2 flex items-center justify-center gap-1 whitespace-nowrap ${userPreference.data.riskLevel === 'high' ? 'bg-[#ffe2e5]' : 'bg-[#c3e7f2]'
                              }`}>
                              <div className={`w-5 h-5 rounded-full flex items-center justify-center ${userPreference.data.riskLevel === 'high' ? 'bg-[#ff616d]' : 'bg-[#006ffd]'
                                }`}>
                          ...
                          <>
                            ...
                            <div className="flex items-center justify-center gap-2">
                              ...
                              {isLoggedIn && (
                                ...
                                >
                                  <Heart
                                    className={`w-4 h-4 cursor-pointer transition-colors ${favoriteStocks.some(stock => stock.symbol === aiRecommendations[selectedAiIndex].symbol)
