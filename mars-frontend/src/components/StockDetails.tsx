...
import { getStockDetails, getStockNews } from "../api/stockApi"
import type { StockDetails as StockDetailsType, NewsItem } from "../types/stock"
...
const StockDetails: React.FC<StockDetailsProps> = ({ symbol, activeTab, onTabChange }) => {
                  ...
                  news.map((item) => (
                    <div key={item.id} className="flex gap-3">
                      <div className="w-14 h-14 md:w-16 md:h-16 bg-[#f5f7f9] rounded-xl flex items-center justify-center">
                        <img
                          src={item.imageUrl || "/assets/financial-news.png"}
                          alt="뉴스 이미지"
                          width={56}
                          height={56}
